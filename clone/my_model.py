...
class LSTM_ngram(nn.Module):
    ...
    def padding(self, x):
        ...
        for i in range(len(lens)):
            if max_len-lens[i]:
                x[i].extend([0] * (max_len-lens[i]))
        return x#Variable(self.th.LongTensor(x)), lens#Variable(self.th.LongTensor(lens))
    
    def embeddings(self, x):
        for i in range(len(x)):
            for j in range(len(x[i])):
                if x[i][j] == 0:
                    x[i][j] = np.zeros(self.embedding_dim)
                else:
                    x[i][j] = self.ngram.getVector(x[i][j])
        return Variable(self.th.FloatTensor(x))
